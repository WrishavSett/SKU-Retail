...
class LLMReranker:
    def __init__(self, model_name: str = "gemini-2.5-flash-lite", api_key: str = None):
        """
        Standalone LLM Re-ranker for transaction matches using Google Gemini
...
Where X is the rank number (1, 2, 3, etc.) or "0" for no good match."""
  ...
  MRP: {mrp}"""
    ...
    def query_llm(self, prompt: str) -> Optional[str]:
        """Query Google Gemini LLM and return selected rank"""
        try:
            ...
            logger.error(f"Error querying Google Gemini LLM: {e}")
            return None
    ...
    def rerank_csv_matches(self, csv_file_path: str, output_file_path: str) -> pd.DataFrame:
        ...
        # Group by all transaction-prefixed columns to avoid dropping rows
